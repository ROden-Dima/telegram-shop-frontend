...
function Checkout() {
    ...
    imageList.length &&
      mutationPhotos.mutate(
        ...
        },
        {
          ...
          onError: () => {
            message.error("Произошла ошибка при загрузке изображения, пожалуйста, попробуйте снова");
            setImagesLoading(false);
          }
        }
      );
  ...
  useEffect(() => {
    if (!data || !data?.addresses) {
      ...
      message.warning({
        content: "Вы должны добавить адрес перед оплатой",
        duration: 3
      });
    }
  }, [data, error]);
    ...
    <Container title="Оплата и оформление заказа" backwardUrl={-1}>
      <div>
        <Alert
          message={
            <div>
              <div className="mb-2"> Информация о счете для перевода суммы</div>
        ...
        <br />
        <Alert
          message="Загрузка фото квитанции об оплате обязательна. После загрузки вы можете оформить заказ"
          type="warning"
          showIcon
        />
        ...
        <Form
            ...
            if (receiptPhoto) {
              mutationOrder.mutate(
                ...
                },
                {
                  onSuccess: () => {
                    message.success(
                      "Ваш заказ успешно оформлен. Вы можете отслеживать свой заказ в аккаунте"
                    );
                    navigate("/");
                  }
                }
              ...
              message.error("Пожалуйста, загрузите фото квитанции об оплате");
            }
          }}
          ...
          <Spin spinning={imagesLoading} tip="Загрузка">
            <Form.Item
              ...
              name="photos"
              label="Фото квитанции об оплате"
              required
